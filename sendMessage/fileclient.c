...
// int main()
{
    ...
    inet_pton(AF_INET, "192.168.160.243", &serverAddr.sin_addr.s_addr);
    ...
    while (1)
    {
        ...
        {
            send(clientSocket, "get 0x01", sizeof("get 0x01"), 0);
            printf("hello\n");
        }
        else if (strcmp(buffer, "2") == 0)
        {
            send(clientSocket, "get 0x02", sizeof("get 0x01"), 0);
            printf("world\n");
        }
        else if (strcmp(buffer, "3") == 0)
        {
            send(clientSocket, "get 0x03", sizeof("get 0x01"), 0);
            receiveFile(clientSocket, "./medium/");
        ...
        {
            // next
            send(clientSocket, "get 0x04", sizeof("get 0x01"), 0);
            st=1;
        }
        else if (strcmp(buffer, "5") == 0)
        {
            // back
            send(clientSocket, "get 0x05", sizeof("get 0x01"), 0);
            st=2;
        }
        else if (strcmp(buffer, "6") == 0)
        {
            // stop or continue
            send(clientSocket, "get 0x06", sizeof("get 0x01"), 0);
            st=3;
        }
        else if (strcmp(buffer, "7") == 0)
        {
            // stop or continue
            send(clientSocket, "get 0x07", sizeof("get 0x01"), 0);
            st=5;
        }
        else
            break;
